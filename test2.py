...
parser.add_argument("--data_dir", default="./dataset/base_train/", type=str, help="dataset directory")
...
#    val_loader = get_loader(args)
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            print("inputs:", test_inputs.shape) # torch.Size([1, 1, 280, 219, 224])
            img_name = batch["image_meta_dict"]["filename_or_obj"][0].split("/")[-1]
            names = img_name.split('.')[0]
#            print(names)
            
            val_outputs = sliding_window_inference(test_inputs, (96, 96, 96), 4, model, overlap=args.infer_overlap)
#            print('slide',val_outputs.shape) #torch.Size([1, 12, 280, 219, 224])
            val_outputs = torch.softmax(val_outputs, 1).cpu().numpy()
#            print('soft',val_outputs.shape) #  (1, 12, 271, 221, 314)

            val_outputs = np.argmax(val_outputs, axis=1).astype(np.uint8)
#            val_outputs= np.squeeze(val_outputs, 0)
            print('argmax',val_outputs.shape)

#            print(val_outputs.astype)#(1, 271, 221, 314)

#            val_outputs = nib.Nifti1Image(val_outputs, affine=np.eye(4))
#            print('nib',val_outputs.shape)
#            nib.save(val_outputs, img_name)
